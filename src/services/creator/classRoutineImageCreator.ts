...
import { RoutineOptions, ClassSchedule } from "../../types/types";
...
export default function classRoutineImageCreator({
  ...
  timeSlots,
  weeklySchedule,
}: RoutineOptions & { weeklySchedule: ClassSchedule[] }) {
  ...
  // Find the maximum slot number that's actually used
  const maxUsedSlot = Math.max(
    ...usedDayRoutines.flatMap((day) => day.classes.map((cls) => cls.slot))
  );

  // Only use time slots up to the maximum used slot
  const usedTimeSlots = timeSlots.slice(0, maxUsedSlot);

  const cellWidth = 95;
  const rowHeight = 90;
  const padding = 60;
  const tableWidth = cellWidth * (usedTimeSlots.length + 1);
  ...
  for (let i = 0; i <= usedDays.length; i++) {
    for (let j = 0; j <= usedTimeSlots.length; j++) {
      ...
      if (i === 0 && j > 0) {
        ...
        const timeSlotText = usedTimeSlots[j - 1];
